...
// Importamos las funciones para cerrar sesión de Firebase
...
export default function PokedexScannerScreen() {
  ...
  const handleLogout = async () => {
    await signOut(auth);
    // Le decimos explícitamente que nos lleve a la pantalla de bienvenida
    router.replace('/'); 
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.customButton} onPress={toggleCamera}>
          <Text style={styles.buttonText}>{isCameraActive ? 'Detener' : 'Escanear'}</Text>
        </TouchableOpacity>
        <Link href="/pokedex" asChild>
          <TouchableOpacity style={[styles.customButton, styles.greenButton]}>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#EE1515', padding: 20, justifyContent: 'space-around', borderWidth: 10, borderColor: '#C40D0D', },
  logoutButton: { position: 'absolute', top: 60, right: 20, backgroundColor: 'rgba(0,0,0,0.4)', paddingHorizontal: 15, paddingVertical: 8, borderRadius: 20, zIndex: 10, },
  logoutButtonText: { color: 'white', fontWeight: 'bold', },
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', },
  title: { fontSize: 55, fontWeight: 'bold', color: '#FFCC00', textShadowColor: 'rgba(0, 0, 0, 0.8)', textShadowOffset: { width: 3, height: 3 }, textShadowRadius: 10, alignSelf: 'center', marginTop: -10, marginBottom: 5, },
  cameraLens: { width: 70, height: 70, backgroundColor: '#3D7DCA', borderRadius: 35, borderWidth: 6, borderColor: '#fff', },
  indicatorLights: { flexDirection: 'row', marginLeft: 20, gap: 10, },
  light: { width: 25, height: 25, borderRadius: 12.5, borderWidth: 2, borderColor: 'rgba(0,0,0,0.2)', },
  screenFrame: { width: '100%', height: '45%', backgroundColor: '#DEDEDE', borderRadius: 20, padding: 15, borderBottomRightRadius: 60, justifyContent: 'center', alignItems: 'center', overflow: 'hidden', },
  ...
  customButton: { width: '45%', paddingVertical: 18, borderRadius: 15, backgroundColor: '#3D7DCA', borderWidth: 3, borderColor: 'rgba(255, 255, 255, 0.5)', shadowColor: "#000", shadowOffset: { width: 0, height: 4, }, shadowOpacity: 0.30, shadowRadius: 4.65, elevation: 8, alignItems: 'center', justifyContent: 'center', },
  greenButton: { backgroundColor: '#50AF5D', },
